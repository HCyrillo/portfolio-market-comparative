...
module.exports = (productService) => {
  ...
  router.post('/', [body('name').isString().trim().notEmpty().withMessage('Nome é obrigatório.'), body('category').isString().trim().notEmpty().withMessage('Categoria é obrigatória.'), available, checkExact([], { message: 'Body contém campos não permitidos.' })], validate, asyncHandler(async (req, res) => success(res, await productService.create(req.body), 201)));
